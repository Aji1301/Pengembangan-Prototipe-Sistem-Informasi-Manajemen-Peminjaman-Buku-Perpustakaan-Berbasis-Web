...
function Profile({ user, onLogout }: { user?: any; onLogout: () => void }) {
  ...
  const nisValue = u.nisNip || u.nim || u.idNumber || "2026101";
  ...
  const handleSaveProfile = () => {
    const updatedUser = { ...u, name, phone, kelas };
    localStorage.setItem("kancil_user", JSON.stringify(updatedUser));
    setEdit(false);
        ...
        <div className="bg-gradient-to-r from-forest to-forest-deep px-8 py-8 text-paper flex flex-wrap items-center justify-between gap-6">
          <div className="flex items-center gap-5">
            ...
            <div>
              ...
              <p className="text-paper/80 text-sm font-600 mt-0.5">{u.email || "siswa@kancil.com"}</p>
            </div>
          </div>

          <Button variant="secondary" onClick={() => setEdit(!edit)} className="bg-white text-forest hover:bg-paper font-800">
            <Icon.edit className="w-5 h-5" /> {edit ? "Tutup Editor" : "Edit Profil"}
          </Button>
        </div>
        ...
        <div className="p-8 space-y-6">
          ...
          {edit && (
            <div className="pt-4 flex justify-end gap-3 border-t border-border">
              ...
              <Button onClick={handleSaveProfile} size="lg" className="px-7">
                <Icon.check className="w-5 h-5" /> Simpan Perubahan Profil
              </Button>
            </div>
          )}
        </div>
